function scan_data = read_LSN_data(path)
    ...
    % Start position
    start = [0 0 0];
    start(2) = fscanf(fid, '%%Sample Y (um): %f'); fseek(fid,2,0);
    start(1) = fscanf(fid, '%%Initial X (um): %f'); fseek(fid,2,0);
    start(3) = fscanf(fid, '%%Initial Z (um): %f'); fseek(fid,2,0);
    ...
    sz(2) = fscanf(fid, '%%Scan size Z (um): %f'); fseek(fid,2,0);
    scan_data.image_size = sz;
    
    % Number of pixels
    pixels = [0 0];
    pixels(1) = fscanf(fid, '%%Number of X pixels: %d'); fseek(fid,2,0);
    pixels(2) = fscanf(fid, '%%Number of Z pixels: %d'); fseek(fid,2,0);
...
%Fix the x and y axes being backwards
scan_data.start=[-scan_data.start(1),-scan_data.start(2),scan_data.start(3)];
scan_data.step_size=-scan_data.step_size;
scan_data.image = flipud(scan_data.image);
