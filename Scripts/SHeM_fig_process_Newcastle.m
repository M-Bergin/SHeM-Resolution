...
%Add label
% if sb_pos==0
%     t_h=text(x_location+Scalebar_length/2,y_lims(2)+(vert_shift*2.2)*abs(diff(y_lims)),[num2str(Scalebar_length),' ',char(181),'m'],'Color','y','FontSize',18,'HorizontalAlignment','center');
% elseif sb_pos==1
%     t_h=text(x_location-Scalebar_length/2,y_lims(1)-(vert_shift*2)*abs(diff(y_lims)),[num2str(Scalebar_length),' ',char(181),'m'],'Color','y','FontSize',18,'HorizontalAlignment','center');
% end
%% Tidy up the figure
